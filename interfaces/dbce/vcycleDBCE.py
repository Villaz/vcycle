...
import os
from VCYCLE import logLine, tenancies, lastFizzles
import uuid
...
class VcycleDBCE(vcycleBase):
    ...
    def _create_client(self):
        ...
        dbce_client = interfaces.dbce.client.DBCE(tenancy['url'], tenancy['key'], tenancy['version'])
        return dbce_client

    def _servers_list(self):
        '''Returns a list of all servers created and not deleted in the tenancy'''
        servers_list = self.client.machine.list()
        return servers_list
    ...
    def _update_properties(self, server, vmtypeName,runningPerVmtype, notPassedFizzleSeconds, properties, totalRunning):
        ...
        if server.status == 'SHUTOFF' and (properties['updatedTime'] - properties['startTime']) < tenancy['vmtypes'][vmtypeName]['fizzle_seconds']:
            logLine(tenancy_name, server.name + ' was a fizzle! ' + str(properties['updatedTime'] - properties['startTime']) + ' seconds')
        try:
            lastFizzles[tenancy_name][vmtypeName] = properties['updatedTime']
        except:
